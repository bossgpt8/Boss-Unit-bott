...
import { LayoutDashboard, Settings, Terminal, Shield, Cpu, LogOut } from "lucide-react";
      ...
      <header className="fixed top-0 w-full z-50 border-b border-white/10 bg-black/80 backdrop-blur-2xl">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight leading-none text-white">BOSS<span className="text-primary">BOT</span></h1>
              <span className="text-[10px] text-muted-foreground font-medium tracking-wider uppercase">Advanced WhatsApp Platform</span>
            </div>
          </div>
          ...
          <nav className="flex items-center gap-1">
            ...
            {user && (
              <div className="flex items-center gap-3 ml-4 pl-4 border-l border-white/10">
                ...
                <span className="text-xs text-gray-400 hidden md:block">{user.displayName}</span>
                <button
                  onClick={() => logout()}
