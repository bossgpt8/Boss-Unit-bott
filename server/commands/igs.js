...
async function igsCommand(sock, chatId, message, crop = false) {
    try {
        ...
        const downloadDir = path.join(tmpDir, `igs_${Date.now()}`);
        fs.mkdirSync(downloadDir, { recursive: true });

        const downloadCommand = `yt-dlp --output "${downloadDir}/%(id)s.%(ext)s" "${urlMatch[0]}"`;
        ...
        const downloadedFiles = fs.readdirSync(downloadDir).map(file => path.join(downloadDir, file));
        ...
        const maxItems = Math.min(downloadedFiles.length, 10);
