...
async function pmblockerCommand(sock, chatId, senderId, mentionedJids, message, args) {
    ...
    if (!sub || !['on', 'off', 'status', 'setmsg'].includes(sub.toLowerCase())) {
        await sock.sendMessage(chatId, { text: '*PM BLOCKER*\n\n.pmblocker on - Block all DMs except owner\n.pmblocker off - Allow DMs\n.pmblocker status - Show status\n.pmblocker setmsg <text> - Set warning message' }, { quoted: message });
        return;
    }
