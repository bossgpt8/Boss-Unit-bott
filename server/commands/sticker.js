...
async function stickerCommand(sock, chatId, senderId, mentionedJids, message, args) {
    ...
    try {
        ...
        if (!mediaBuffer) {
            await sock.sendMessage(chatId, { text: 'Failed to download media. Please try again.' }, { quoted: messageToQuote });
            return;
        }
        ...
        if (isAnimated && webpBuffer.length > 1000 * 1024) {
            try {
                ...
                const fallbackCmd = isLargeFile
                    ? `ffmpeg -y -i "${tempInput}" -t 2 -vf "scale=512:512:force_original_aspect_ratio=decrease,fps=8,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=#00000000" -c:v libwebp -preset default -loop 0 -vsync 0 -pix_fmt yuva420p -quality 30 -compression_level 6 -b:v 100k "${tempOutput2}"`
                    : `ffmpeg -y -i "${tempInput}" -t 3 -vf "scale=512:512:force_original_aspect_ratio=decrease,fps=12,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=#00000000" -c:v libwebp -preset default -loop 0 -vsync 0 -pix_fmt yuva420p -quality 45 -compression_level 6 -b:v 150k "${tempOutput2}"`;
                await new Promise((resolve, reject) => {
        ...
        const json = {
            ...
            'sticker-pack-name': settings.packname || 'BossBot',
            'emojis': ['🤖']
        };
        ...
        if (isAnimated && finalBuffer.length > 900 * 1024) {
            try {
                ...
                const smallCmd = `ffmpeg -y -i "${tempInput}" -t 2 -vf "scale=320:320:force_original_aspect_ratio=decrease,fps=8,pad=320:320:(ow-iw)/2:(oh-ih)/2:color=#00000000" -c:v libwebp -preset default -loop 0 -vsync 0 -pix_fmt yuva420p -quality 30 -compression_level 6 -b:v 80k "${tempOutput3}"`;
                await new Promise((resolve, reject) => {
        ...
        await sock.sendMessage(chatId, { text: 'Failed to create sticker! Try again later.' }, { quoted: message });
    }
}
